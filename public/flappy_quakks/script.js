...
// — Asset containers and loading
const IMG = { bg:[], pipe:[], bird:[], nums:[], base:null, msg:null, over:null };
const AUD = {};
let loadedImages = 0;
const TOTAL_IMAGES =
  SPRITES.bg.length + SPRITES.pipe.length + 1 +
  SPRITES.bird.length + SPRITES.nums.length + 1 + 1;
...
function startPlay(){
  state='PLAY';
  score=0; bird.vy=0; bird.x=WIDTH*0.2;
  bird.y=(HEIGHT-bird.h)/2; spawnTimer=-SPAWN_INT;
  lastTime=performance.now(); lastDifficultyScore=0;
  difficultyCycle=0; PIPE_GAP=180; PIPE_SPEED=200; SPAWN_INT=1.5;
  spawnInitial(); AUD.wing.play();
}

// — MAIN LOOP
function gameLoop(){
  if(state==='PLAY'){ updatePlay(); drawPlay(); }
  else if(state==='GAMEOVER'){ drawGameOver(); }
  // LEADERBOARD state is drawn in fetchLeaderboard
}
...
function updatePlay(){
  ...
  if(score>=25 && score%25===0 && score>lastDifficultyScore && !pipes.some(p=>!p.scored)){
    ...
    difficultyCycle++; lastDifficultyScore=score;
  }
  // spawn & move pipes
  spawnTimer+=dt;
  if(spawnTimer>=SPAWN_INT){ spawnPipe(); spawnTimer-=SPAWN_INT; }
  const pv=PIPE_SPEED*dt; pipes.forEach(p=>p.x-=pv);
...
async function handleGameOver(){
  ...
  state='GAMEOVER'; AUD.hit.play(); AUD.die.play();

  // submit score & fetch leaderboard
  const origin = window.location.origin;
  const submitUrl      = `${origin}/flappy_quakks/submit`;
  const leaderboardUrl = `${origin}/flappy_quakks/leaderboard`;
